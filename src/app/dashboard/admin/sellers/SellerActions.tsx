    ...
    <div className="flex justify-end gap-2">
      {banned ? (
        ...
        <button
          disabled={loading}
          onClick={() => {
            if (confirm("Ban this seller?"))
              patch({ banned: true, status: "inactive", banReason: "Banned by admin" });
          }}
      ...
      {status === "active" && !banned ? (
        <button
          ...
          className="rounded-md bg-[#F0E6D2] px-2.5 py-1 text-xs font-medium text-[#3A342C] disabled:opacity-50"
        >
          Deactivate
        </button>
      ) : !banned ? (
