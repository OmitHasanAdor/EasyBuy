...
type NotificationItem = {
  id: string;
  type: "order" | "shipping" | "review" | "promo" | "wishlist" | "system";
  title: string;
  message: string;
  read: boolean;
  createdAt: string;
...
// Placeholder — later connect to Notification model + API
  ...
  return (
    <div className="space-y-6 p-6 md:p-8">
      ...
      <div className="rounded-lg border border-[#E7DCC4] bg-[#F7F2E7] px-4 py-3 text-sm text-[#3A342C]">
        Notification system is not connected to the database yet. When enabled,
        you will get alerts for order status, shipping, reviews, and offers.
      </div>
      ...
      {notifications.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-[#E7DCC4] bg-white py-16">
          ...
          <p className="mt-1 max-w-sm text-center text-sm text-[#8E3D14]/70">
            You&apos;ll be notified about order updates, shipping, wishlist
            items on sale, and special offers.
          </p>
        </div>
      ) : (
              ...
              >
                <div
                  className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-full ${
                    n.read
                      ? "bg-[#F0E6D2] text-[#8E3D14]"
                      : "bg-[#C05620]/15 text-[#C05620]"
                  }`}
                >
